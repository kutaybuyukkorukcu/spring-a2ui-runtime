...
/**
 * Inbound translator from A2UI-like payloads into FogUI canonical contract. v1 scope intentionally
 * supports a conservative subset and emits explicit fallback blocks for unknown nodes.
 */
public class A2UiInboundTranslator {
  ...
  private List<ThinkingItem> translateThinking(
      Object rawThinking, List<A2UiTranslationError> errors) {
  ...
  private ThinkingItem translateThinkingItem(
      Object raw, int index, List<A2UiTranslationError> errors) {
  ...
  private List<ContentBlock> translateContent(
      Object rawContent, List<A2UiTranslationError> errors, String path) {
  ...
  private ContentBlock translateBlock(
      Object rawBlock, String path, List<A2UiTranslationError> errors) {
    ...
    if ("text".equalsIgnoreCase(type)) {
      ...
      if (value == null) {
        errors.add(
            error(path + ".value", FogUiErrorCode.MISSING_TEXT, "text block requires value/text"));
        value = "";
      }
    ...
    if (componentType != null) {
      ...
      List<ContentBlock> children =
          rawChildren == null ? null : translateContent(rawChildren, errors, path + ".children");

      Object rawProps = map.get("props");
      Map<String, Object> props = rawProps instanceof Map<?, ?> typed ? castMap(typed) : Map.of();
    ...
    errors.add(
        error(path, FogUiErrorCode.UNSUPPORTED_NODE, "unsupported A2UI node, fallback emitted"));
