...
public class StreamPatchReconciler {
  ...
  public GenerativeUIResponse reconcile(
      GenerativeUIResponse previous, GenerativeUIResponse incoming) {
    if (incoming == null) {
      return previous;
    }
  ...
  private List<ThinkingItem> chooseThinking(
      List<ThinkingItem> previous, List<ThinkingItem> incoming) {
  ...
  private List<ContentBlock> chooseContent(
      List<ContentBlock> previous, List<ContentBlock> incoming) {
    if (incoming != null && !incoming.isEmpty()) {
      return new ArrayList<>(incoming);
    }
    if (previous != null) {
  ...
  private Map<String, Object> chooseMetadata(
      Map<String, Object> previous, Map<String, Object> incoming) {
  ...
  private GenerativeUIResponse normalize(GenerativeUIResponse source) {
    ...
    normalized.setThinking(
        source.getThinking() == null ? new ArrayList<>() : new ArrayList<>(source.getThinking()));
    normalized.setContent(
        source.getContent() == null ? new ArrayList<>() : new ArrayList<>(source.getContent()));
    normalized.setMetadata(
        source.getMetadata() == null ? null : new HashMap<>(source.getMetadata()));
    return normalized;
  }
}
