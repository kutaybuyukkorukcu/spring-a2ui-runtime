...
class A2UiCompatibilityFixtureTest {
  ...
  void shouldTranslateFixtureDeterministically(String fixtureName, A2UiFixture fixture) {
    ...
    for (int index = 0; index < fixture.expectedBlocks().size(); index++) {
      ...
      if (expectedBlock.componentType() != null) {
        assertEquals(
            expectedBlock.componentType(),
            actualBlock.getComponentType(),
            "component type should match fixture");
      }
      if (expectedBlock.value() != null) {
        assertEquals(
            expectedBlock.value(), actualBlock.getValue(), "text value should match fixture");
      }
      if (expectedBlock.fallbackReason() != null) {
        Map<?, ?> props = (Map<?, ?>) actualBlock.getProps();
        assertEquals(
            expectedBlock.fallbackReason(),
            props.get("reason"),
            "fallback reason should match fixture");
      }
    }
  }
  ...
  static Stream<org.junit.jupiter.params.provider.Arguments> fixtureCases()
    ...
    Path fixturesRoot =
        Path.of(
            A2UiCompatibilityFixtureTest.class
                .getClassLoader()
  ...
  private record FixtureBlock(
      String type, String componentType, String value, String fallbackReason) {}
