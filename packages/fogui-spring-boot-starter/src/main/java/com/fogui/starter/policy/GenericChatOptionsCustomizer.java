...
public class GenericChatOptionsCustomizer implements FogUiChatOptionsCustomizer {
  ...
  private void copyGenericOptions(
      ChatOptions.Builder builder, @Nullable ChatOptions incomingOptions) {
  ...
  private void applyPolicy(ChatOptions copiedOptions, @NonNull FogUiGenerationPolicy policy) {
    applyTextIfPresent(
        policy.getModel(), value -> setIfPresent(copiedOptions, "setModel", String.class, value));
    applyIfPresent(
        policy.getTemperature(),
        value -> setIfPresent(copiedOptions, "setTemperature", Double.class, value));
    applyIfPresent(
        policy.getTopP(), value -> setIfPresent(copiedOptions, "setTopP", Double.class, value));
    applyIfPresent(
        policy.getMaxTokens(),
        value -> setIfPresent(copiedOptions, "setMaxTokens", Integer.class, value));
  }
  ...
  private <T> void setIfPresent(
      Object target, String methodName, Class<T> parameterType, @Nullable T value) {
    if (value == null) {
      return;
    }
    ...
    try {
      ...
      throw new IllegalStateException(
          "Failed to apply deterministic option "
              + methodName
              + " on "
