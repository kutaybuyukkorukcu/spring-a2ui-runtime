...
class FogUiChatOptionsPolicyApplierTest {
    ...
    void shouldCreateOpenAiOptionsWhenProviderConfiguredInEnvironment() {
        FogUiChatOptionsPolicyApplier applier = newApplier(
                new MockEnvironment().withProperty("spring.ai.openai.chat.options.model", "gpt-4.1-nano"),
                defaultProperties());
        ...
        assertThat(openAiOptions.getResponseFormat().getType()).isEqualTo(ResponseFormat.Type.JSON_OBJECT);
    ...
    void shouldCreateAzureOpenAiOptionsWhenProviderConfiguredInEnvironment() {
        FogUiChatOptionsPolicyApplier applier = newApplier(
                new MockEnvironment().withProperty("spring.ai.azure.openai.chat.options.deployment-name", "fogui-gpt4o"),
                defaultProperties());
    ...
    void shouldCreateAnthropicOptionsWhenProviderConfiguredInEnvironment() {
        FogUiChatOptionsPolicyApplier applier = newApplier(
                new MockEnvironment().withProperty("spring.ai.anthropic.chat.options.model", "claude-3-5-sonnet"),
                defaultProperties());
    ...
    void shouldCreateGeminiOptionsWhenProviderConfiguredInEnvironment() {
        FogUiChatOptionsPolicyApplier applier = newApplier(
                new MockEnvironment().withProperty("spring.ai.vertex.ai.gemini.chat.options.model", "gemini-2.5-pro"),
                defaultProperties());
