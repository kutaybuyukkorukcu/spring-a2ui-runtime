package com.kutaybuyukkorukcu.a2ui.runtime.error;

public enum A2UiErrorCode {

    NULL_MESSAGE_BATCH("NULL_MESSAGE_BATCH", A2UiErrorCategory.VALIDATION),
    NULL_MESSAGE("NULL_MESSAGE", A2UiErrorCategory.VALIDATION),
    INVALID_MESSAGE_ENVELOPE("INVALID_MESSAGE_ENVELOPE", A2UiErrorCategory.PROTOCOL),
    INVALID_MESSAGE_SEQUENCE("INVALID_MESSAGE_SEQUENCE", A2UiErrorCategory.PROTOCOL),
    UNKNOWN_ROOT_COMPONENT("UNKNOWN_ROOT_COMPONENT", A2UiErrorCategory.PROTOCOL),
    MISSING_SURFACE_ID("MISSING_SURFACE_ID", A2UiErrorCategory.VALIDATION),
    MISSING_COMPONENT_ID("MISSING_COMPONENT_ID", A2UiErrorCategory.VALIDATION),
    INVALID_COMPONENT_DEFINITION("INVALID_COMPONENT_DEFINITION", A2UiErrorCategory.VALIDATION),
    INVALID_COMPONENT_PAYLOAD("INVALID_COMPONENT_PAYLOAD", A2UiErrorCategory.VALIDATION),
    MISSING_REQUIRED_PROP("MISSING_REQUIRED_PROP", A2UiErrorCategory.VALIDATION),
    UNKNOWN_PROP("UNKNOWN_PROP", A2UiErrorCategory.VALIDATION),
    INVALID_BOUND_VALUE("INVALID_BOUND_VALUE", A2UiErrorCategory.VALIDATION),
    INVALID_ENUM_VALUE("INVALID_ENUM_VALUE", A2UiErrorCategory.VALIDATION),
    INVALID_CHILD_SHAPE("INVALID_CHILD_SHAPE", A2UiErrorCategory.VALIDATION),
    INVALID_ACTION_SHAPE("INVALID_ACTION_SHAPE", A2UiErrorCategory.VALIDATION),
    INVALID_PROP_TYPE("INVALID_PROP_TYPE", A2UiErrorCategory.VALIDATION),
    UNKNOWN_COMPONENT_TYPE("UNKNOWN_COMPONENT_TYPE", A2UiErrorCategory.CATALOG),
    UNSUPPORTED_CATALOG_ID("UNSUPPORTED_CATALOG_ID", A2UiErrorCategory.CATALOG),
    MISSING_CATALOG_ID("MISSING_CATALOG_ID", A2UiErrorCategory.VALIDATION),
    MISSING_ROOT("MISSING_ROOT", A2UiErrorCategory.VALIDATION),
    INVALID_DATA_UPDATE("INVALID_DATA_UPDATE", A2UiErrorCategory.PROTOCOL),
    INVALID_DATA_ENTRY("INVALID_DATA_ENTRY", A2UiErrorCategory.PROTOCOL),
    UNSUPPORTED_VERSION("UNSUPPORTED_VERSION", A2UiErrorCategory.PROTOCOL),
    PARSE_ERROR("PARSE_ERROR", A2UiErrorCategory.PROTOCOL),

    CONTENT_REQUIRED("CONTENT_REQUIRED", A2UiErrorCategory.VALIDATION),
    TRANSFORM_FAILED("TRANSFORM_FAILED", A2UiErrorCategory.RUNTIME),
    TRANSFORM_PARSE_FAILED("TRANSFORM_PARSE_FAILED", A2UiErrorCategory.RUNTIME),
    TOOL_CHOICE_UNAVAILABLE("TOOL_CHOICE_UNAVAILABLE", A2UiErrorCategory.RUNTIME),
    NO_COMPATIBLE_CATALOG("NO_COMPATIBLE_CATALOG", A2UiErrorCategory.CATALOG),
    A2UI_VALIDATION_FAILED("A2UI_VALIDATION_FAILED", A2UiErrorCategory.VALIDATION),

    INVALID_CLIENT_EVENT("INVALID_CLIENT_EVENT", A2UiErrorCategory.PROTOCOL),
    INVALID_USER_ACTION("INVALID_USER_ACTION", A2UiErrorCategory.PROTOCOL),
    ACTION_NOT_HANDLED("ACTION_NOT_HANDLED", A2UiErrorCategory.RUNTIME),
    UNKNOWN_ACTION("UNKNOWN_ACTION", A2UiErrorCategory.RUNTIME),
    CONFIRMATION_REQUIRED("CONFIRMATION_REQUIRED", A2UiErrorCategory.RUNTIME),
    COMPONENT_NOT_ALLOWED("COMPONENT_NOT_ALLOWED", A2UiErrorCategory.RUNTIME),
    INVALID_ACTION_RESPONSE("INVALID_ACTION_RESPONSE", A2UiErrorCategory.VALIDATION);

    private final String code;
    private final A2UiErrorCategory category;

    A2UiErrorCode(String code, A2UiErrorCategory category) {
        this.code = code;
        this.category = category;
    }

    public String code() {
        return code;
    }

    public A2UiErrorCategory category() {
        return category;
    }
}