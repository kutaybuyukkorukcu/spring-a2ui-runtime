...
class A2UiActionControllerRouteMappingTest {
    ...
    void setUp() {
        ...
        when(actionService.handleClientEvent(any(), eq("req-action-route-1"))).thenReturn(A2UiActionResponse.builder()
                .accepted(true)
    ...
    void shouldExposeA2UiActionsRoute() throws Exception {
        mockMvc.perform(post("/a2ui/actions")
                ...
                .andExpect(header().string(RequestCorrelationService.REQUEST_ID_HEADER, "req-action-route-1"));
    ...
    void shouldRejectFogUiActionsRoute() throws Exception {
        mockMvc.perform(post("/fogui/actions")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {"userAction":{"name":"confirm","surfaceId":"booking","sourceComponentId":"submit-btn","timestamp":"2026-05-01T19:00:00Z","context":{}}}
                                """))
