...
public final class TransformPrompts {
  ...
  private static final String RUNTIME_CANONICAL_RESPONSE_REMINDER =
      """
            Runtime canonical response rules for A2UI v0.8:
            ...
            - This canonical JSON is the runtime's internal response format and will be validated and mapped to A2UI v0.8 messages.
            """;
  ...
  public static final String TRANSFORM_SYSTEM_PROMPT =
      """
        You are a UI transformation engine for a Spring A2UI runtime. Your task is to take raw text content and convert it into the runtime's canonical JSON response, which will be validated and mapped to A2UI v0.8 messages.
            ...
            """
          + RUNTIME_CANONICAL_RESPONSE_REMINDER
          + """
  ...
  public static String buildTransformPrompt(String content, String contextHints) {
    return buildTransformPrompt(new TransformPromptContext(content, contextHints, null, null));
  }

  /** Build a complete prompt with content and A2UI runtime context. */
  public static String buildTransformPrompt(TransformPromptContext promptContext) {
    StringBuilder prompt = new StringBuilder();
    prompt.append("Transform the following content into the runtime's canonical JSON for A2UI v0.8:\n\n");
    prompt.append("---\n");
    prompt.append(promptContext.content());
    prompt.append("\n---\n\n");

    if (promptContext.contextHints() != null && !promptContext.contextHints().isEmpty()) {
      prompt.append("Additional context: ").append(promptContext.contextHints()).append("\n");
    }
    if (promptContext.selectedCatalogId() != null && !promptContext.selectedCatalogId().isBlank()) {
      prompt
          .append("Selected A2UI catalog: ")
          .append(promptContext.selectedCatalogId())
          .append(". Use only componentType values supported by this catalog.\n");
    }
    if (promptContext.clientSupportedCatalogIds() != null
        && !promptContext.clientSupportedCatalogIds().isEmpty()) {
      prompt
          .append("Client-supported A2UI catalogs: ")
          .append(String.join(", ", promptContext.clientSupportedCatalogIds()))
          .append("\n");
    }
    if ((promptContext.contextHints() != null && !promptContext.contextHints().isEmpty())
        || (promptContext.selectedCatalogId() != null
            && !promptContext.selectedCatalogId().isBlank())
        || (promptContext.clientSupportedCatalogIds() != null
            && !promptContext.clientSupportedCatalogIds().isEmpty())) {
      prompt.append("\n");
    }

    prompt.append(RUNTIME_CANONICAL_RESPONSE_REMINDER);
