...
public class A2UiActionController {
    ...
    public ResponseEntity<?> handleClientEvent(
        ...
        try {
            ...
            ResponseEntity.BodyBuilder bodyBuilder = response.getMessages() == null || response.getMessages().isEmpty()
            ...
            HttpStatus status = A2UiActionErrorCodes.ACTION_NOT_HANDLED.equals(ex.getErrorCode())
                    ? HttpStatus.UNPROCESSABLE_ENTITY
                    : HttpStatus.BAD_REQUEST;
            return ResponseEntity.status(status)
                    .header(RequestCorrelationService.REQUEST_ID_HEADER, requestId)
                    .body(A2UiErrorResponse.builder()
                            .error(ex.getMessage())
