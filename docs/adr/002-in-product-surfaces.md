...
Surfaces carry **surface state** (values widgets display and actions submit). The host supplies truth via request `context`, their tools/services, or host `assemble`. We never query their system of record. Open-ended utterances (e.g. weather in a chat) need **host tools or context**, not runtime internet. Inventing smoke values from “general knowledge” is demo-only and must be labeled.
