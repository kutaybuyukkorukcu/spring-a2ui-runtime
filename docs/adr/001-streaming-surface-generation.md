...
spring-a2ui is an OSS Spring **GenUI backend runtime** for [A2UI](https://a2ui.org/). We abstract GenUI backend infrastructure so OSS product builders can focus on product — they keep design systems and FE; we own compose → validate → stream → fail-fast over A2UI-native SSE. See [`docs/platform.md`](../platform.md).
